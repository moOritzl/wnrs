...
function selectLevel(level) {
    switch (level) {
        case 1:
            currentDeck = [...levelOne];
            break;
        case 2:
            currentDeck = [...levelTwo];
            break;
        case 3:
            currentDeck = [...levelThree];
            break;
    }
    shuffle(currentDeck);
    discardPile = [];
...
function drawNextCard() {
    if (currentDeck.length === 0) {
        document.getElementById('current-card').innerText = "Keine Karten mehr im Stapel.";
        return;
    }
    const nextCard = currentDeck.shift();
    const currentCardEl = document.getElementById('current-card');
    if (currentCardEl.innerText && currentCardEl.innerText !== "Bitte wähle ein Level.") {
        discardPile.push(currentCardEl.innerText);
        updateDiscardList();
    }
    currentCardEl.innerText = nextCard;
}
